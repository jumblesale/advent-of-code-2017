...
valid1 :: String -> Bool
valid1 x = (length $ nub w) == (length w)
    where w = words x

solution :: (String -> Bool) -> Int
solution f = length $ filter (==True) $ map f $ lines input 

solution1 :: Int
solution1 = solution valid1

valid2 :: String -> Bool
valid2 x = (length $ nub w) == (length w)
    where w = map sort $ words x

solution2 :: Int
solution2 = solution valid2
